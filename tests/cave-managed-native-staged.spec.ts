...
interface ManagedTransport {
  ...
  familiarAnalytics?: (
    familiarId: string,
    options: { recentLimit?: number; window?: '7d' | '14d' | '8w' | 'all' },
    context?: OperationContext,
  ) => Promise<ManagedResponse>;
}
...
describe('managed native Cave client', () => {
  ...
  test('parses the familiar contract and analytics through the native bridge', async () => {
    const report = {
      specVersion: '0.1.0',
      pass: true,
    ...
    const ward = {
      version: '0.1.0',
      protectedFiles: ['SOUL.md', 'ward.toml'],
      invariants: [],
      editablePaths: ['notes/'],
      approvalTiers: { auto: ['read files'], humanReview: ['publish a finding'] },
    };
    let observedAnalyticsOptions: unknown;
    let observedAnalyticsContext: OperationContext | undefined;
    const familiarContract = vi.fn((familiarId: string) =>
      Promise.resolve({
        statusCode: 200,
        payload: successEnvelope(
          {
            contract: {
              ...
              identity: { name: 'Scribe', creature: 'Archivist familiar' },
              ward,
              report,
            },
          },
          {
            ...
            operations: ['familiars.contract.read'],
          },
        ),
      }),
    );
    const familiarAnalytics = vi.fn(
      (
        _familiarId: string,
        options: { recentLimit?: number; window?: string },
        context?: OperationContext,
      ) => {
        observedAnalyticsOptions = options;
        observedAnalyticsContext = context;
        return Promise.resolve({
          statusCode: 200,
          payload: successEnvelope(
            {
              analytics: {
                ...
                windows: {
                  '7d': {
                    ...
                    harnesses: [],
                    coverage: {},
                    days: [{ date: '2026-08-18', completed: 0, failed: 1, cancelled: 0 }],
                  },
                },
                recentAttempts: [],
    ...
    const client = createManagedClient({ familiarContract, familiarAnalytics });
    ...
    expect(familiarContract).toHaveBeenCalledWith('scribe', expect.anything());

    const analytics = await client.familiarAnalytics('scribe', { window: '7d', recentLimit: 5 });
    expect(analytics.windows['7d']?.days).toEqual([
      { date: '2026-08-18', completed: 0, failed: 1, cancelled: 0 },
    ]);
    expect(observedAnalyticsOptions).toEqual({ recentLimit: 5, window: '7d' });
    expect(observedAnalyticsContext?.signal).toBeInstanceOf(AbortSignal);

    // A bridge whose Cave does not advertise the family: refused, not empty.
    const older = createManagedClient({
      familiarContract: () =>
        Promise.resolve({
          statusCode: 200,
          payload: successEnvelope(
            { contract: { id: 'scribe', present: {}, report } },
            { capabilities: ['familiars'], operations: ['familiars.list'] },
          ),
        }),
    });
    const refused: unknown = await older.familiarContract('scribe').then(
      () => undefined,
      (error: unknown) => error,
    );
    expect(refused).toBeInstanceOf(Error);
    expect(['invalid_response', 'service_unavailable']).toContain(
      (refused as { code?: unknown }).code,
    );

    // A bridge that does not implement the read at all is reported, not crashed.
    await expect(createManagedClient({}).familiarAnalytics('scribe')).rejects.toMatchObject({
