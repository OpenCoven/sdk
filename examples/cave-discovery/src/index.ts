...
async function runUnixExample(): Promise<void> {
  ...
  try {
    ...
    const cave = await createDiscoveredCaveClient({
      discovery: {
          ...
          return Promise.resolve(
            new Response(
              JSON.stringify({
                ...
                data: {
                  instanceId: 'example-cave-instance',
    ...
    const health = await cave.health({ timeoutMs: 1_000 });

    if (
      cave.discovery?.source !== 'coven_cave_home' ||
      cave.discovery.instanceId !== 'example-cave-instance' ||
      health.status !== 'ok'
    ) {
