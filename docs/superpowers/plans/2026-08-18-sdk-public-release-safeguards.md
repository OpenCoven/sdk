...
**Status:** Completed and merged to `main` as [`2291002`](https://github.com/OpenCoven/sdk/commit/22910021a558da375c0ff9a015e82301f03ecdcb) on 2026-08-18.

## Completion record

- PR [#7](https://github.com/OpenCoven/sdk/pull/7) passed CI and CodeQL, then
  squash-merged after clean-worktree validation.
- `pnpm install --frozen-lockfile`, build, lint, typecheck, 84 Vitest tests,
  contract verification, packed-package verification, pre-commit, Gitleaks,
  audit, and `git diff --check` passed in the clean worktree.
- GitHub reports zero open CodeQL alerts and zero open Dependabot alerts after
  the `main` workflows completed.
...
- [x] **Step 2: Run the focused tests and confirm the missing permission and override fail**
...
- [x] **Step 3: Add the minimal security configuration**
...
- [x] **Step 4: Confirm the dependency and tests are green**
...
- [x] **Step 1: Write the failing package-manifest assertion**
...
- [x] **Step 4: Prove the standard publish path is blocked**
...
- [x] **Step 1: Write a failing ESLint-boundary test**
...
- [x] **Step 3: Add the narrow ESLint ignore**
...
- [x] **Step 4: Prove both the regression and full lint pass**
...
- Test: `tests/package-manifests.spec.ts`
- Verify: no additional production source changes

- [x] **Step 1: Preserve declaration build ordering in the root verifier**

The clean checkout showed that typed lint ran before the declaration-producing
build. Add and run a failing assertion that `scripts.verify` begins with build
then lint, then set the command to:
...
- [x] **Step 3: Recheck hosted findings after the patch is committed and pushed**
...
- [x] **Step 4: Commit, publish, and merge after explicit approval**

The implementation was committed as `6fa109a`, published as PR #7, and
squash-merged as `2291002`. Branch protection was intentionally left unchanged.
