...
These examples are deterministic and perform no external network access.
`cave-discovery` creates a temporary owner-local Client v1 record and injects
its health response on Unix; Windows runs must supply a reviewed native
filesystem trust provider, so the portable example reports a skip there. The
other examples use caller-supplied transports.
