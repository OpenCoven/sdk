...
describe('unresolved SDK #38 conformance gaps', () => {
  ...
  test('freezes the exact candidate, manifest, package, source, and consumer bytes', () => {
    ...
    expect(lock.sources).toMatchObject({
      ...
      coven: {
        commit: 'd56021851537d62cfad9ece80583a908f20898a1',
        tree: '65cc9b3fc5458ffc6745324997a5a56679c675c3',
      },
      chat: {
  ...
  test('freezes the compatible Chat schema-v2 producer and protected workflow', () => {
    ...
    expect(lock.evidenceProducer).toEqual({
      status: 'compatible',
      repository: 'OpenCoven/chat',
      commit: '3d5af5b3441991e0314c4f2dfa91bfe9b450e9a7',
      tree: '2d7b73e62796b8468443584a0b1286ca53f66b24',
      packageManifest: {
        path: 'package.json',
        size: 4_044,
        sha256:
      ...
      harness: {
        ...
        size: 205138,
        sha256:
          '7dfe2cc2bf7deafec6749e3801075e238cb2d3fe5ccff6ad60e2fa9a36277c4d',
      },
      command: 'test:phase1-conformance',
      recordSchemaVersion: 2,
      workflow: {
        ...
        size: 472_838,
        sha256:
          '5789ab5653e36226bd459bc7ebe62bbb802d8c11fbfbddc6fc5d0ae9481c1a80',
        job: 'platform-conformance',
        jobNameTemplate: 'platform-conformance ({platform})',
        aggregationJob: 'aggregate-conformance',
        aggregationJobName: 'aggregate-conformance',
        ...
        windowsBootstrapScriptSha256:
          'c40303d9a69a0e7a3e679ed6c4d2da8dcdaf54e91cfdbf19c440d9d0250f4fba',
        ...
        unixSupervisorPreparationScriptSha256:
          '71289faa3baab9f5af9ed08a62fd6b5c9e1ceafec13dffdfa99554652f35eb63',
        unixToolPathSource: {
          path: 'scripts/executable-resolution.mjs',
          size: 9_154,
          sha256:
        ...
        signerWorkflow:
          'OpenCoven/chat/.github/workflows/client-v1-conformance.yml',
        signerDigest: '3d5af5b3441991e0314c4f2dfa91bfe9b450e9a7',
        sourceDigest: '3d5af5b3441991e0314c4f2dfa91bfe9b450e9a7',
        predicateType: 'https://slsa.dev/provenance/v1',
        denySelfHostedRunners: true,
      },
    });
  ...
  test('requires full checkout history so the locked Chat harness revision is available', () => {
    ...
    expect(sha256(TEST_WINDOWS_CHILD_BOOTSTRAP)).toBe(
      '06724d766f6fc6339a9547f641113cd75d36828317b67e81a14e1fb02ebdf971',
    );
    expect(() =>
      verifyProtectedWorkflow(
