...
`e6bf92c6ec47fd6a5e859afa17db314618b1c2c5`, pinning harness source
`719cb5d6e0653956773743522b9267b87a5e9b05` and the unchanged Cave revision
`cb3d22d1f403dd3b94b02668a599a2bf94999e8b`. The actual merge preserves the
tested tree and signed frozen source ancestry. It retains the published
`service-unavailable` launch identifier and six additional native launch codes,
and confirms already-exited retained Windows process handles while preserving
live denial and exit-code verification.

Protected run `34746733029` passed Linux and macOS but failed Windows quota
directory enumeration under `cave-checkout` before launch. Validation,
aggregation, and attestation were skipped. Fresh protected evidence remains
required after this validator lands and both scopes are rotated.
...
An observed `service_unavailable` result does not by itself prove a timeout.
